...
<body>
    ...
    {{-- Page 1: Quotation --}}
    <table class="letterhead small-text-head">
        <tr>
            <td width="20%" valign="top">
                <img src="{{ public_path(str_replace('public/', 'storage/', $data['company_logo'])) }}" alt="Company Logo"
                    style="width:150px;">
            </td>
            <td width="55%" valign="top" class="small-text-head">
        ...
        <table>
            <tr>
                <td width="50%" valign="top">
                    Date: {{ date('d F Y', strtotime($data['quotation_date'])) }}<br><br>
    ...
    <div class="section small-text">
        System Features: Rate Modulation, Auto Capture Pacing System, Extended Hysteresis, Rest Rate,
    ...
    <div class="section small-text">
        ...
        Delivery: Subject to prior sales otherwise 8–12 weeks upon receipt of confirmed order<br>
        Validity: {{ date('d F Y', strtotime($data['quotation_date'])) }} -
        {{ date('d F Y', strtotime('+1 year', strtotime($data['quotation_date']))) }} (1 year)<br>
        Payment: By bank draft made payable to "{{ $data['company_name'] }}"
    </div>
    ...
    <div class="section">
        <p>
            ...
            <strong style="text-transform: capitalize;">{{ $data['company_name'] }}</strong> whose registered office is
            at {{ $data['company_address'] }} as our agent to supply Cardiac Rhythm Management
            (CRM) products to all MOH hospitals in Malaysia.
        </p>
        <br>
        <p>
            <strong style="text-transform: capitalize;">{{ $data['company_name'] }}</strong> is therefore authorized by
